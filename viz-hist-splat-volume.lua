...
_G.densityRange = vec2f(math.huge, -math.huge)
...
_G.logDensityRange = densityRange:map(function(x) return math.log(x + logDensityEpsilon) end)
print('log density range', logDensityRange)

_G.logDensityAlphaRange = vec3f(0, 1, 1)	-- .x = min, .y = max, .z = clamped scale
...
void main() {
	tc = vec2(vertex.x, 1. - vertex.y);
	gl_Position = mvProjMat * vec4(
		mix(volumeMin, volumeMax, vec3(vertex, 0.)),
		1.
	);
}
]],
...
function App:update()
	...
	if self.mapObj then
		self.mapObj.uniforms.mvProjMat = self.view.mvProjMat.ptr
		self.mapObj.uniforms.volumeMin = volumeRange.min.s
		self.mapObj.uniforms.volumeMax = volumeRange.max.s
		self.mapObj:draw()
	end
